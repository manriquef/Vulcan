export const reportScore = ({collection, item, forceUpdate}) => {
  ...
  // If for some reason item doesn't have a "reportedAt" property, abort
  if (!item.reportedAt)
    return 0;

  const reportedAt = item.reportedAt.valueOf();
  const now = new Date().getTime();
  const age = now - reportedAt;
  const ageInHours = age / (60 * 60 * 1000);

  if (reportedAt > now) // if post has been scheduled in the future, don't update its score
    return 0;
  ...
  const reportBaseScore = item.reportBaseScore || 0;

  // HN algorithm
  const newScore = reportBaseScore / Math.pow(ageInHours + 2, f);
